...
const salesByCustomer = [
  { customer: "Tech Solutions Sdn Bhd", orders: 0, sales: 0 },
  { customer: "Global Trading Co", orders: 0, sales: 0 },
  { customer: "Innovative Systems", orders: 0, sales: 0 },
  { customer: "Office Hub Malaysia", orders: 0, sales: 0 },
  { customer: "Modern Enterprises", orders: 0, sales: 0 },
  { customer: "Digital Works Sdn Bhd", orders: 0, sales: 0 },
  { customer: "Smart Office Solutions", orders: 0, sales: 0 },
  { customer: "Premier Business Group", orders: 0, sales: 0 },
  { customer: "Muzahid Khan", orders: 0, sales: 0 },
  { customer: "Arif R.", orders: 0, sales: 0 },
];
...
const salesColumns: ColumnDef<SalesCustomer>[] = [
  ...
  { accessorKey: "sales", header: "Sales (RM)", cell: ({ row }) => <>RM {(row.getValue("sales") as number).toFixed(2)}</> },
];
...
const arColumns: ColumnDef<AR>[] = [
  { accessorKey: "invoiceNumber", header: "Invoice #", cell: ({ row }) => <span className="font-medium">{row.getValue("invoiceNumber")}</span> },
  { accessorKey: "customer", header: "Customer" },
  { accessorKey: "date", header: "Date" },
  { accessorKey: "due", header: "Due" },
  { accessorKey: "total", header: "Total", cell: ({ row }) => <>RM {(row.getValue("total") as number).toFixed(2)}</> },
  { accessorKey: "paid", header: "Paid", cell: ({ row }) => <>RM {(row.getValue("paid") as number).toFixed(2)}</> },
  { accessorKey: "balance", header: "Balance", cell: ({ row }) => {
    const val = row.getValue("balance") as number;
    return <Badge className={val > 0 ? "bg-red-500" : "bg-green-500"}>RM {val.toFixed(2)}</Badge>;
  }},
];
...
export default function CustomerReports() {
  ...
  <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
    ...
    {/* Filter Button */}
    <Button className="mt-2 sm:mt-6 self-start sm:self-auto">Filter</Button>
  </div>
</div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <Card>
          ...
          <CardContent>
            <DataTable columns={salesColumns} data={salesByCustomer} />
          </CardContent>
        </Card>
