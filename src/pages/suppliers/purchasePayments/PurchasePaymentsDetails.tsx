    ...
    <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in-50 duration-500 pb-10 px-4 md:px-0">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 pt-4">
        <div>
          ...
          <div className="flex items-center gap-3">
            <h1 className="text-3xl font-bold tracking-tight text-gray-900 dark:text-gray-100 italic">
              Payment #PPAY-{payment.id.toString().padStart(6, "0")}
            </h1>
        ...
        <div className="flex flex-wrap items-center gap-2">
          {payment.invoice && (
            <Link to={`/dashboard/purchase-invoices/${payment.invoice_id}`}>
              <Button variant="outline" className="bg-white hover:bg-slate-50 border-slate-200 gap-2">
                <FileText className="w-4 h-4" /> View Invoice {payment.invoice.invoice_number}
              </Button>
            </Link>
          )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-6">
                  <DetailItem
                    ...
                    value={`PPAY-${payment.id.toString().padStart(6, "0")}`}
                  />
                  <DetailItem
                    icon={<CreditCard className="w-4 h-4 text-violet-500" />}
                    label="Payment Method"
                    value={payment.payment_method.replaceAll("_", " ").toUpperCase()}
                    isBadge
                  />
                  <DetailItem
                ...
                <div className="space-y-6">
                  <DetailItem
                    ...
                    label="Date Recorded"
                    value={new Date(payment.payment_date).toLocaleDateString(undefined, { dateStyle: 'long' })}
                  />
                  <DetailItem
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          {/* Supplier Snapshot */}
          {payment.purchase_order?.supplier && (
            <Card className="shadow-sm border-border/60 overflow-hidden">
              ...
              <CardContent className="pt-6 space-y-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-bold text-sm">{payment.purchase_order.supplier.name}</p>
                ...
                <div className="space-y-3">
                  <div className="flex items-center gap-3 text-sm text-muted-foreground">
                    ...
                    <span className="truncate">{payment.purchase_order.supplier.email}</span>
                  </div>
                  <div className="flex items-center gap-3 text-sm text-muted-foreground">
                    <Phone className="w-4 h-4 shrink-0" />
                    <span>{payment.purchase_order.supplier.phone}</span>
                  </div>
                  {payment.purchase_order.supplier.contact_person && (
                    <div className="flex items-center gap-3 text-sm text-muted-foreground group/link">
                      ...
                      <span className="group-hover/link:text-foreground transition-colors">{payment.purchase_order.supplier.contact_person}</span>
                    </div>
                  )}
                </div>
                <div className="grid grid-cols-2 gap-2 pt-2">
                  <Button variant="outline" size="sm" className="h-8 text-xs font-semibold hover:bg-indigo-50 hover:text-indigo-600 hover:border-indigo-200 transition-all" asChild>
                    <a href={`mailto:${payment.purchase_order.supplier.email}`}>
                      <Mail className="w-3 h-3 mr-1.5" /> Email
                    </a>
                  </Button>
                  <Button variant="outline" size="sm" className="h-8 text-xs font-semibold hover:bg-indigo-50 hover:text-indigo-600 hover:border-indigo-200 transition-all" asChild>
                    <a href={`tel:${payment.purchase_order.supplier.phone}`}>
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center text-sm p-3 bg-slate-50 dark:bg-slate-900 rounded-lg group">
                  <div className="flex flex-col">
                    ...
                    <span className="font-bold">#{payment.purchase_order?.po_number}</span>
                  </div>
                  <Link to={`/dashboard/purchase-orders/${payment.purchase_order_id}`}>
                ...
                {payment.invoice && (
                  <div className="flex justify-between items-center text-sm p-3 bg-slate-50 dark:bg-slate-900 rounded-lg group">
                    <div className="flex flex-col">
                      ...
                      <span className="font-bold">#{payment.invoice.invoice_number}</span>
                    </div>
                    <Link to={`/dashboard/purchase-invoices/${payment.invoice_id}`}>
                      <Button variant="ghost" size="sm" className="h-8 w-8 p-0 opacity-0 group-hover:opacity-100 transition-opacity">
                        <ArrowLeft className="h-4 w-4 rotate-180" />
