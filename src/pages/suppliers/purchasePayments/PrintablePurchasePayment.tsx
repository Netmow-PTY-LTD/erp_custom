...
export default function PrintablePurchasePayment({ payment, to }: Props) {
    ...
    const from = payment?.purchase_order?.supplier;
            ...
            <div id="payment-receipt" className="invoice-box print:border-0 print:p-0">
                ...
                <div className="flex justify-between items-start mb-4">
                    ...
                    <div className="text-right flex flex-col items-end">
                        ...
                        <h2 className="font-bold text-gray-800 mb-1 uppercase details-text">Payment Voucher</h2>
                        <div className="details-text space-y-1">
                            <p><strong>Date:</strong> {formatDate(payment?.payment_date || "")}</p>
                            <p><strong>Receipt No.:</strong> PPAY-{payment?.id.toString().padStart(6, "0")}</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2 mb-4">
                    <div className="border border-gray-300">
                        <div className="bg-gray-100 px-3 py-1 font-bold details-text border-b border-gray-300">Payment From</div>
                    ...
                    <div className="border border-gray-300">
                        <div className="bg-gray-100 px-3 py-1 font-bold details-text border-b border-gray-300">Payment To (Supplier)</div>
                        <div className="p-3 details-text min-h-[80px]">
                            <p className="font-bold">{from?.name}</p>
                            <p className="whitespace-pre-line">{(from as any)?.address}</p>
                ...
                <div className="w-full mb-6 text-sm">
                    <table className="w-full border-collapse table-border">
                        ...
                        <tbody>
                            <tr>
                                <td>
                                    <div className="font-bold">Payment for Purchase Order #{payment?.purchase_order?.po_number}</div>
                                    {payment?.invoice && (
                                        <div className="text-xs text-gray-600 mt-1">Invoice Reference: {payment.invoice.invoice_number}</div>
                                    )}
                                    <div className="text-xs text-gray-600 mt-1">Payment Method: {payment?.payment_method.replaceAll("_", " ").toUpperCase()}</div>
                                    {payment?.reference_number && (
                                        <div className="text-xs text-gray-600">Reference: {payment.reference_number}</div>
                                    )}
                                </td>
                        ...
                        <table className="w-full details-text font-bold border-collapse">
                            <tbody>
                                <tr className="border border-gray-300 bg-gray-50 text-sm">
                                    <td className="p-1 px-4 text-left border-r border-gray-300 uppercase">Paid Amount</td>
