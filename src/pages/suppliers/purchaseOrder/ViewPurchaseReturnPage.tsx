...
export default function ViewPurchaseReturnPage() {
    ...
    const { data, isLoading } = useGetPurchaseReturnByIdQuery(Number(returnId));
    ...
    const handleCreateInvoice = async () => {
        try {
            ...
            if (res.status && res.data?.id) {
                ...
                navigate(`/dashboard/purchase-return-invoices/${res.data.id}`);
            } else if (res.status) {
                toast.success(res.message || "Return Invoice created successfully");
            }
        } catch {
                ...
                <div>
                    ...
                    <div className="flex items-center gap-3">
                        <h1 className="text-3xl font-bold tracking-tight text-gray-900 dark:text-gray-100">
                            Purchase Return #{purchaseReturn.return_number || purchaseReturn.po_number || purchaseReturn.id}
                        </h1>
                    ...
                    {purchaseReturn.po_number && (
                        <p className="text-sm text-muted-foreground mt-1">
                            Based on Purchase Order: <span className="font-semibold text-primary">#{purchaseReturn.po_number}</span>
                        </p>
                    )}
                </div>
                    ...
                    {(purchaseReturn.status === 'approved' || purchaseReturn.status === 'returned') && (
                        <>
                            {purchaseReturn.invoice ? (
                                <Link to={`/dashboard/purchase-return-invoices/${purchaseReturn.invoice.id}`}>
                        ...
                        <Card className="shadow-sm border-border/60">
                            <CardContent className="p-4 flex flex-col gap-1">
                                ...
                                <span className="text-sm font-semibold flex items-center gap-2">
                                    <Calendar className="w-4 h-4 opacity-50" /> {formatDateStandard(purchaseReturn?.return_date || purchaseReturn?.order_date)}
                                </span>
                            </CardContent>
                        </Card>
                        <Card className="shadow-sm border-border/60">
                            <CardContent className="p-4 flex flex-col gap-1">
                                ...
                                <span className="text-sm font-semibold flex items-center gap-2">
                                    <Package className="w-4 h-4 opacity-50" /> {(purchaseReturn as any)?.purchase_order?.po_number || purchaseReturn.po_number || "N/A"}
                                </span>
                            </CardContent>
                        </Card>
                    </div>
                                        ...
                                        purchaseReturn.items.map((item: any) => {
                                            ...
                                            return (
                                                <tr key={item.id} className="hover:bg-muted/20 transition-colors">
                                                    ...
                                                    <td className="px-6 py-4 text-right">{currency} {Number(item.unit_cost || 0).toFixed(2)}</td>
                        ...
                        <CardContent className="space-y-4">
                            <div className="space-y-2 text-sm">
                                <div className="flex justify-between items-center text-muted-foreground">
                                    <span>Subtotal</span>
                                    <span>{currency} {Number(purchaseReturn.total_amount || 0).toFixed(2)}</span>
                                </div>
                                <div className="flex justify-between items-center text-muted-foreground">
                                    <span>Discount</span>
                                    <span className="text-red-500">- {currency} {Number(purchaseReturn.discount_amount || 0).toFixed(2)}</span>
                                </div>
                                <div className="flex justify-between items-center text-muted-foreground">
                                    <span>Tax</span>
                                    <span>+ {currency} {Number(purchaseReturn.tax_amount || 0).toFixed(2)}</span>
                                </div>

                                <Separator className="my-2" />

                                {(() => {
                                    const totalVal = Number(purchaseReturn?.grand_total || purchaseReturn?.total_payable_amount || (Number(purchaseReturn?.total_amount || 0) + Number(purchaseReturn?.tax_amount || 0) - Number(purchaseReturn?.discount_amount || 0)));

                                    const payments = purchaseReturn?.payments || [];
                                    const calculatedTotalPaid = payments.reduce((acc: number, p: any) => acc + Number(p.amount || 0), 0);

                                    const paidVal = Number(purchaseReturn?.total_refunded_amount || calculatedTotalPaid || 0);
                                    const balanceVal = Number(purchaseReturn?.due_refund_amount ?? (totalVal - paidVal));
