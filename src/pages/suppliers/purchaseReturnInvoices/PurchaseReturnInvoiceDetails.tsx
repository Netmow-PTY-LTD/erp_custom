...
export default function PurchaseReturnInvoiceDetails() {
    ...
    const { data, isLoading } = useGetPurchaseReturnInvoiceByIdQuery(id as string);
    ...
    const subtotal = purchaseReturn?.total_amount || 0;
    const tax = purchaseReturn?.tax_amount ?? 0;
    const discount = purchaseReturn?.discount_amount ?? 0;
    const payments = invoice?.payments || [];
    const calculatedTotalPaid = payments.reduce((acc: number, p: any) => acc + Number(p.amount || 0), 0);

    const totalRefundable = Number(purchaseReturn?.grand_total || purchaseReturn?.total_payable_amount || 0);
    const totalRefunded = Number(purchaseReturn?.total_refunded_amount || calculatedTotalPaid || 0);
        ...
        <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in-50 duration-500 pb-10">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                ...
                <div className="flex flex-wrap items-center gap-2">
                    {balance > 0 && (
                        <Link to={`/dashboard/purchase-returns/payments/create?rn=${(purchaseReturn as any)?.return_number}`}>
                            <Button className="gap-2 shadow-sm bg-orange-600 hover:bg-orange-700">
                                <CreditCard className="w-4 h-4" /> Record Refund
                            </Button>
                        </Link>
                    )}
                    <Link to={`/dashboard/purchase-return-invoices/${invoice?.id}/preview`}>
                        ...
                        <Card className="shadow-sm border-border/60">
                            <CardContent className="p-4 flex flex-col gap-1">
                                ...
                                <span className="text-sm font-semibold flex items-center gap-2">
                                    <Hash className="w-4 h-4 opacity-50" /> {(purchaseReturn as any)?.purchase_order?.po_number || purchaseReturn?.po_number || '-'}
                                </span>
                            </CardContent>
                        </Card>
                        <Card className="shadow-sm border-border/60">
                            <CardContent className="p-4 flex flex-col gap-1">
                                ...
                                <span className="text-sm font-semibold flex items-center gap-2">
                                    <Calendar className="w-4 h-4 opacity-50" /> {formatDateStandard(invoice?.invoice_date)}
                                </span>
                            </CardContent>
                        </Card>
                        <Card className="shadow-sm border-border/60">
                            <CardContent className="p-4 flex flex-col gap-1">
                                ...
                                <span className="text-sm font-semibold text-red-600 flex items-center gap-2">
                                    <Calendar className="w-4 h-4 opacity-50" /> {formatDateStandard(invoice?.due_date)}
                                </span>
                            </CardContent>
                        </Card>
                        <Card className="shadow-sm border-border/60">
