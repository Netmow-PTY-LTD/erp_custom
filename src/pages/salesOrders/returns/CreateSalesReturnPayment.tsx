    ...
    const SalesReturnSelectField = ({ field }: { field: any }) => {
        ...
        useEffect(() => {
            if (!field.value && list.length > 0 && returnNumberParam) {
                const ret = list.find((r: any) => r.order_number === returnNumberParam);
                        ...
                        {returnDetails ? (
                            <>
                                <div className="space-y-3 pb-4 border-b">
                                    <div className="flex justify-between text-sm"><span className="text-muted-foreground">Return #</span><span className="font-bold">{returnDetails.order_number}</span></div>
                                    <div className="flex justify-between text-sm"><span className="text-muted-foreground">Subtotal</span><span>{currency} {Number(returnDetails.total_amount).toFixed(2)}</span></div>
                                    <div className="flex justify-between text-sm"><span className="text-muted-foreground">Tax</span><span>{currency} {Number(returnDetails.tax_amount).toFixed(2)}</span></div>
                                    <div className="flex justify-between text-sm"><span className="text-muted-foreground text-red-500">Discount</span><span className="text-red-500">- {currency} {Number(returnDetails.discount_amount).toFixed(2)}</span></div>
                                </div>
