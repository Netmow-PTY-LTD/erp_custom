...
export default function SalesReturnInvoiceDetails() {
    ...
    const subtotal = salesReturn?.total_amount || 0;
    const tax = salesReturn?.tax_amount ?? 0;
    const discount = salesReturn?.discount_amount ?? 0;
    const payments = invoice?.payments || [];
    const calculatedTotalPaid = payments.reduce((acc: number, p: any) => acc + Number(p.amount || 0), 0);

    const totalRefundable = Number(salesReturn?.grand_total || 0);
    const totalRefunded = Number(salesReturn?.total_refunded_amount || calculatedTotalPaid || 0);
    const balance = Number(salesReturn?.due_refund_amount ?? (totalRefundable - totalRefunded));
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div className="text-left">
                    ...
                    <div className="flex items-center gap-3">
                        <h1 className="text-3xl font-bold tracking-tight text-gray-900 dark:text-gray-100">Return Invoice #{invoice?.invoice_number}</h1>
                            ...
                            <div className="space-y-4 text-left">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="font-semibold text-gray-900">{settingsInfo?.company_name}</p>
                                ...
                                <tbody className="divide-y divide-border">
                                    {salesReturn?.items?.map((item: any) => (
                                        <tr key={item.id} className="hover:bg-muted/20 transition-colors">
                                            ...
                                            <td className="px-6 py-4 text-right font-medium">{currency} {Number(item.unit_price || 0).toFixed(2)}</td>
                                            ...
                                            <td className="px-6 py-4 text-right font-bold text-gray-900">{currency} {(Number(item.line_total || 0) + Number(item.tax_amount || 0)).toFixed(2)}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
