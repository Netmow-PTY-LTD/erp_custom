...
import { useAddSalesReturnMutation } from "@/store/features/salesOrder/salesReturnApiService";
import { useGetAllSalesOrdersQuery, useGetSalesOrderByIdQuery } from "@/store/features/salesOrder/salesOrder";
import { useNavigate, useSearchParams } from "react-router";
import { useGetCustomersQuery, useGetCustomerByIdQuery } from "@/store/features/customers/customersApi";
...
export default function CreateSalesReturn() {
    ...
    const { data: salesOrderData } = useGetSalesOrderByIdQuery(watchSalesOrderId ?? 0, { skip: !watchSalesOrderId });
        ...
        if (salesOrderData?.data) {
            ...
            if (order.items && Array.isArray(order.items)) {
                const newItems = order.items.map((item: any) => ({
                    ...
                    sales_tax: Number(item.sales_tax_percent) || 0,
    ...
    function CustomerSelectField({ field }: { field: { value?: number; onChange: (v: number) => void } }) {
    ...
    function SalesOrderSelectField({ field, customerId, setValue }: { field: any, customerId: number, setValue: UseFormSetValue<SalesOrderFormValues> }) {
        const [open, setOpen] = useState(false);
        const [query, setQuery] = useState("");

        const { data, isLoading } = useGetAllSalesOrdersQuery({ page: 1, limit: 100, search: query, customer_id: customerId }, { skip: !customerId });
        const { data: singleData } = useGetSalesOrderByIdQuery(Number(field.value), { skip: !field.value });

        const list = (Array.isArray(data?.data) ? data.data : []).filter((so: any) => Number(so.customer_id) === Number(customerId));
        const selected = list.find((so: any) => Number(so.id) === Number(field.value)) ||
            (singleData?.data?.id && Number(singleData.data.id) === Number(field.value) ? singleData.data : undefined);
            ...
            <Popover open={open} onOpenChange={setOpen}>
                <PopoverTrigger asChild>
                    <Button variant="outline" role="combobox" disabled={!customerId} className="w-full justify-between h-9 px-3 text-left overflow-hidden">
                        <span className="truncate text-sm">{selected ? (selected.order_number || `SO-${selected.id}`) : "Select Sales Order..."}</span>
                        <ChevronDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                    </Button>
                </PopoverTrigger>
                        ...
                        <CommandList className="max-h-[300px]">
                            ...
                            <CommandGroup>
                                ...
                                {!isLoading && list.map((so: any) => (
                                    <CommandItem key={so.id} onSelect={() => { field.onChange(Number(so.id)); setValue("orderNumber", so.order_number); setOpen(false); }} className="flex items-center gap-2 cursor-pointer">
                                        <div className="flex flex-col flex-1">
                                            <span className="font-medium text-sm">{so.order_number}</span>
                                            <span className="text-xs text-muted-foreground">Date: {new Date(so.order_date).toLocaleDateString()} | Total: {so.total_payable_amount}</span>
                                        </div>
                                    </CommandItem>
                                ))}
                            </CommandGroup>
    ...
    function ProductSelectField({ field, index, setValue }: { field: any, index: number, setValue: UseFormSetValue<SalesOrderFormValues> }) {
        const [open, setOpen] = useState(false);
        const [query, setQuery] = useState("");

        const { data, isLoading } = useGetAllProductsQuery({ page: 1, limit: 50, search: query });
        const { data: singleData } = useGetProductByIdQuery(Number(field.value), { skip: !field.value });
                ...
                <PopoverTrigger asChild>
                    <Button variant="outline" role="combobox" className="w-full justify-between h-9 overflow-hidden text-left">
                        <div className="flex items-center gap-2 overflow-hidden flex-1 min-w-0">
                            {selected && <Avatar className="h-6 w-6 shrink-0"><AvatarImage src={selected.thumb_url} /><AvatarFallback><Package className="h-3 w-3" /></AvatarFallback></Avatar>}
    ...
    const onSubmit = async (values: SalesOrderFormValues) => {
        try {
            ...
            const payload = {
                ...
                ...(values.salesOrderId && { order_id: Number(values.salesOrderId) }),
                ...(values.orderNumber && { order_number: values.orderNumber }),
                        ...
                        <CardHeader className="bg-gradient-to-r from-blue-50 via-indigo-50 to-blue-50 dark:from-blue-950/30 dark:via-indigo-950/30 dark:to-blue-950/30 border-b-1 border-blue-100 dark:border-blue-900 py-3 gap-0">
                            <div className="flex flex-wrap items-center justify-between gap-4">
                                ...
                                <div className="flex flex-wrap items-center gap-3">
                                    <Button type="button" variant="outline" onClick={() => append({ productId: 0, sku: "", specification: "", unit: "", quantity: 1, unit_price: 0, discount: 0, sales_tax: 0, stock_quantity: 0 })} className="gap-2"><Plus className="w-4 h-4" /> Add Row</Button>
                                ...
                                <div className="space-y-4">
                                    {fields.map((field, index) => (
                                        <div key={field.id} className="flex flex-wrap xl:flex-nowrap min-w-max gap-4 items-start xl:items-center bg-gray-50 p-4 rounded-xl border border-gray-100 dark:bg-gray-900/40 dark:border-gray-800 transition-all duration-200 hover:shadow-md">
                                            ...
                                            <FormField name={`items.${index}.sales_tax`} control={control} render={({ field }) => (
                ...
                onApply={(addedProducts, deselectedIds) => {
                    ...
                    addedProducts.forEach(product => {
                        if (!newItems.some(item => item.productId === product.id)) {
                            newItems.push({ productId: product.id, sku: product.sku || "", specification: product.specification || "", unit: product.unit?.name || "", quantity: 1, unit_price: Number(product.price) || 0, discount: 0, sales_tax: product.sales_tax || 0, stock_quantity: product.stock_quantity || 0 });
                        }
                    });
                    form.setValue("items", newItems);
                }}
