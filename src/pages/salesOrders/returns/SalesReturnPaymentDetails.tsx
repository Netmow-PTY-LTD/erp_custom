...
import { useGetSalesReturnPaymentByIdQuery } from "@/store/features/salesOrder/salesReturnApiService";
        ...
        <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in-50 duration-500 pb-10">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div className="text-left">
                    ...
                    <h1 className="text-3xl font-bold tracking-tight text-blue-700">Refund #RRFD-{payment.id.toString().padStart(6, "0")}</h1>
                </div>
                <div className="flex gap-2">
                    {invoice && (
                        <Link to={`/dashboard/sales-return-invoices/${invoice.id}`}>
                            <Button className="bg-blue-600 hover:bg-blue-700 shadow-md">View Return Invoice {invoice.invoice_number}</Button>
                        </Link>
                    )}
                </div>
            </div>
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-12">
                                <div className="space-y-4">
                                    <div><p className="text-xs font-bold text-muted-foreground uppercase tracking-widest mb-1">Refund Method</p><p className="font-semibold text-gray-900 capitalize">{payment.payment_method.replaceAll("_", " ")}</p></div>
                                    <div><p className="text-xs font-bold text-muted-foreground uppercase tracking-widest mb-1">Reference Number</p><p className="font-semibold text-gray-900">{payment.reference_number || "-"}</p></div>
                                ...
                                <div className="space-y-4">
                                    <div><p className="text-xs font-bold text-muted-foreground uppercase tracking-widest mb-1">Refund Date</p><p className="font-semibold text-gray-900 flex items-center gap-2"><Calendar className="w-4 h-4 opacity-50" /> {formatDateStandard(payment.payment_date)}</p></div>
                ...
                <div className="space-y-6 text-left">
                    ...
                    <div className="rounded-xl border bg-white p-6 shadow-sm space-y-4">
                        ...
                        <div className="space-y-3 text-sm">
                            <div className="flex justify-between items-center"><span className="text-muted-foreground">Return #</span><span className="font-bold text-blue-600 cursor-pointer">{salesReturn?.return_number || "-"}</span></div>
                            <div className="flex justify-between items-center"><span className="text-muted-foreground">Total Refundable</span><span className="font-bold">{currency} {Number(salesReturn?.total_refundable_amount || 0).toFixed(2)}</span></div>
                        </div>
                    </div>
                    ...
                    {invoice && (
                        <div className="rounded-xl border bg-white p-6 shadow-sm space-y-4">
                            ...
                            <div className="space-y-3 text-sm">
                                <div className="flex justify-between items-center"><span className="text-muted-foreground">Invoice #</span><span className="font-bold">{invoice.invoice_number}</span></div>
                                <div className="flex justify-between items-center"><span className="text-muted-foreground">Total Payable</span><span className="font-bold">{currency} {Number(invoice.total_payable_amount || 0).toFixed(2)}</span></div>
                                <div className="flex justify-between items-center"><span className="text-muted-foreground">Due Date</span><span className="font-bold text-red-600">{formatDateStandard(invoice.due_date)}</span></div>
                            </div>
                        </div>
                    )}
                </div>
