...
export default function RecordSalesReturnRefundModal({ open, onOpenChange, defaultReturnId, onSuccess }: Props) {
    ...
    const form = useForm<PaymentFormValues>({
        ...
        defaultValues: {
            sales_return_id: defaultReturnId || 0,
            amount: undefined,
            payment_method: "",
            reference: "",
            notes: "",
    ...
    useEffect(() => {
        if (defaultReturnId) {
            form.setValue("sales_return_id", defaultReturnId);
        }
    ...
    async function onSubmit(values: PaymentFormValues) {
        try {
            ...
            }).unwrap(); // We are not sending date for now as per API, but if API supports it we would add it here.
    ...
    const SalesReturnSelectField = ({ field }: { field: any }) => {
        ...
        const selected = list.find((r: any) => r.id === field.value) || (returnDetails?.id === field.value ? returnDetails : null);
                        ...
                        <CommandList>
                            ...
                            <CommandGroup>
                                ...
                                {!isLoading && list.map((r: any) => (
                                    <CommandItem key={r.id} onSelect={() => { field.onChange(r.id); setOpenCombobox(false); }}>
                                        <div className="flex flex-col text-left w-full">
                                            ...
                                            <span className="text-xs text-muted-foreground mt-0.5">Balance: {currency} {(Number(r?.grand_total || 0) - Number(r?.total_refunded_amount || 0)).toFixed(2)}</span>
                                        </div>
                                    </CommandItem>
                                ))}
                            </CommandGroup>
                ...
                <div className="p-6 pb-2">
                    <DialogHeader className="text-left space-y-1">
                        ...
                        <DialogDescription className="text-base text-muted-foreground">
                            {returnDetails
                                ? `${returnDetails.return_number || `RET-${watchReturnId}`} - ${returnDetails.customer?.name || 'Unknown Customer'}`
                ...
                <div className="px-6 pb-6 space-y-6">
                    ...
                    <Form {...form}>
                        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                <FormField name="payment_date" control={form.control} render={({ field }) => (
                                    <FormItem className="text-left space-y-1.5">
