...
} from "@/store/features/salesOrder/salesReturnApiService";
...
export default function ViewSalesReturn() {
    ...
    const { data, isLoading } = useGetSalesReturnByIdQuery(Number(returnId));
    ...
    const handleApprove = async () => {
        try {
            const res = await updateStatus({ id: salesReturn.id, status: "approved" }).unwrap();
            if (res.status) {
    ...
    const handleCreateInvoice = async () => {
        try {
            const res = await addInvoice({ sales_return_id: salesReturn.id, due_date: new Date().toISOString().split("T")[0] }).unwrap();
            if (res.status && res.data?.id) {
                toast.success(res.message || "Return Invoice created successfully");
                navigate(`/dashboard/sales-return-invoices/${res.data.id}`);
            } else if (res.status) {
                toast.success(res.message || "Return Invoice created successfully");
            }
        } catch {
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div className="text-left">
                    ...
                    <div className="flex items-center gap-3">
                        <h1 className="text-3xl font-bold tracking-tight text-gray-900 dark:text-gray-100">
                            Sales Return #{salesReturn.return_number || salesReturn.id}
                        </h1>
                    ...
                    {salesReturn.sales_order && (
                        <p className="text-sm text-muted-foreground mt-1">
                            Based on Sales Order: <span className="font-semibold text-primary">#{salesReturn.sales_order.order_number}</span>
                        </p>
                    )}
                </div>
                ...
                <div className="flex flex-wrap items-center gap-2">
                    ...
                    {(salesReturn.status === 'approved' || salesReturn.status === 'returned' || salesReturn.status === 'completed') && (
                        <>
                            {salesReturn.invoice ? (
                                <Link to={`/dashboard/sales-return-invoices/${salesReturn.invoice.id}`}>
                                ...
                                <tbody className="divide-y divide-border">
                                    {salesReturn.items?.map((item: any) => (
                                        <tr key={item.id} className="hover:bg-muted/20 transition-colors">
                                            ...
                                            <td className="px-6 py-4 text-right">{currency} {Number(item.unit_price || 0).toFixed(2)}</td>
                                            <td className="px-6 py-4 text-right text-muted-foreground">{currency} {Number(item.discount || 0).toFixed(2)}</td>
                                            <td className="px-6 py-4 text-right text-muted-foreground">{currency} {Number(item.tax_amount || 0).toFixed(2)}</td>
                                            <td className="px-6 py-4 text-right font-bold">{currency} {(Number(item.line_total || 0) + Number(item.tax_amount || 0)).toFixed(2)}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
                    ...
                    <Card className="shadow-md border-border/60 bg-slate-50 dark:bg-slate-900 overflow-hidden pb-6">
                        ...
                        <CardContent className="space-y-4">
                            <div className="space-y-2 text-sm">
                                <div className="flex justify-between items-center text-muted-foreground"><span>Subtotal</span><span>{currency} {Number(salesReturn.total_amount || 0).toFixed(2)}</span></div>
                                <div className="flex justify-between items-center text-muted-foreground"><span>Discount</span><span className="text-red-500">- {currency} {Number(salesReturn.discount_amount || 0).toFixed(2)}</span></div>
                                <div className="flex justify-between items-center text-muted-foreground"><span>Tax</span><span>+ {currency} {Number(salesReturn.tax_amount || 0).toFixed(2)}</span></div>
                                <Separator className="my-2" />
                                {(() => {
                                    const totalVal = Number(salesReturn.grand_total || 0);
                                    const payments = salesReturn.payments || [];
                                    const paidVal = Number(salesReturn.total_refunded_amount || payments.reduce((acc: number, p: any) => acc + Number(p.amount || 0), 0) || 0);
                                    const balanceVal = Number(salesReturn.due_refund_amount ?? (totalVal - paidVal));
                                    return (
                                        <>
