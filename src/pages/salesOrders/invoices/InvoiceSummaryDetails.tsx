...
export default function InvoiceSummaryDetails() {
    ...
    const totalPayable = invoices.reduce((sum, inv) => sum + parseFloat(inv.total_payable || "0"), 0);
    const totalPaid = invoices.reduce((sum, inv) => sum + (inv.paid_amount || 0), 0);
    const totalDue = invoices.reduce((sum, inv) => sum + (inv.remaining_balance || 0), 0);
                        ...
                        <div key={inv.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden group transition-all hover:shadow-md hover:border-gray-200">
                            ...
                            <div className="bg-gray-50/30 p-4 md:p-6 flex flex-wrap justify-end items-end gap-4 md:gap-6 border-t border-gray-100">
                                <div className="flex flex-col items-end">
                                    ...
                                    <span className="text-base md:text-lg font-black text-gray-900">{currency} {parseFloat(inv.total_payable).toFixed(2)}</span>
                                </div>
                                <div className="flex flex-col items-end border-l-0 md:border-l pl-0 md:pl-6 border-gray-200">
                                    ...
                                    <span className="text-base md:text-lg font-black text-emerald-600">{currency} {inv.paid_amount.toFixed(2)}</span>
                                </div>
                                <div className="flex flex-col items-end border-l-0 md:border-l pl-0 md:pl-6 border-gray-200">
                                    <span className="text-[10px] uppercase font-bold text-rose-600 tracking-tighter">Remaining Due</span>
                                    <span className="text-lg md:text-xl font-black text-rose-600 underline decoration-double decoration-rose-500/30 underline-offset-4">{currency} {inv.remaining_balance.toFixed(2)}</span>
                                </div>
                            </div>
                        </div>
                    ))}
