...
export default function StaffAdvanceDetail() {
    ...
    const stats = useMemo(() => {
        ...
        const totalIssued = advances.reduce((sum, adv) => sum + Number(adv.amount), 0);
        const totalReturned = advances.reduce((sum, adv) => sum + Number(adv.returned_amount), 0);
        const pendingBalance = totalIssued - totalReturned;
        const activeCount = advances.filter(adv => (Number(adv.amount) - Number(adv.returned_amount)) > 0).length;
        ...
        <div className="space-y-6">
            <div className="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p className="text-gray-500">Managing advances for <span className="font-semibold text-gray-900">{staff?.first_name} {staff?.last_name}</span> (ID: {id})</p>
                </div>
            </div>
                                        ...
                                        {advancesData?.data && advancesData.data.length > 0 ? (
                                            advancesData.data.map((adv: any) => (
                                                <tr key={adv.id} className="hover:bg-gray-50/50 transition-colors">
                                                    <td className="px-6 py-4 text-gray-500">{adv.advance_date}</td>
                                                    <td className="px-6 py-4 font-bold text-orange-600 text-base">RM {Number(adv.amount).toLocaleString()}</td>
                                                    <td className="px-6 py-4 font-bold text-gray-700 text-base">RM {(Number(adv.amount) - Number(adv.returned_amount)).toLocaleString()}</td>
                        ...
                        <CardContent className="pt-6 space-y-4">
                            <div className="space-y-1.5">
                                ...
                                >
                                    ...
                                    {advancesData?.data?.filter((a: any) => a.status !== 'returned').map((adv: any) => (
                                        <option key={adv.id} value={adv.id}>
                                            Date: {adv.advance_date} | Balance: RM {(Number(adv.amount) - Number(adv.returned_amount)).toLocaleString()}
                                        </option>
                                    ))}
                                </select>
                            </div>
                        ...
                        <CardContent className="p-0">
                            <div className="overflow-x-auto">
                                <table className="w-full text-sm text-left">
                                    ...
                                    <tbody className="divide-y divide-gray-100">
                                        {(() => {
                                            const allReturns = advancesData?.data?.flatMap((adv: any) =>
                                            ...
                                            return allReturns.map((ret: any) => (
                                                <tr key={ret.id} className="hover:bg-gray-50/50 transition-colors">
                                                    <td className="px-6 py-4 text-gray-500">{ret.return_date}</td>
                                                    ...
                                                    <td className="px-6 py-4 text-gray-400 text-right text-xs">Issued On: {ret.advance_date}</td>
                                                </tr>
                                            ));
                                        })()}
                                    </tbody>
