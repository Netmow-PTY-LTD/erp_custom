...
export default function StaffPayrollRun() {
    ...
    // Fetch Staff Data
    const { data: staffResponse, isLoading: isLoadingStaff } = useGetStaffByIdQuery(Number(staffId));
    ...
    const existingPayrollItem = useMemo(() => {
        if (!payrollRunsResponse?.data || !staffId) return null;
        for (const run of (payrollRunsResponse.data as any)) {
            const item = run.items?.find((i: any) => i.staff_id === Number(staffId));
            if (item) return item;
        }
        return null;
    }, [payrollRunsResponse, staffId]);

    useEffect(() => {
        if (existingPayrollItem) {
            setCustomPayrollAmount(String(existingPayrollItem.net_pay));
        } else {
    ...
    const handleProcessPayroll = async () => {
        ...
        try {
            ...
            const response = await fetch('http://localhost:5000/api/payroll/generate', {
                ...
                body: JSON.stringify({
                    month: mString,
                    staff_ids: [Number(viewingAttendanceStaff.id)],
                    custom_amounts: customPayrollAmount ? { [viewingAttendanceStaff.id]: Number(customPayrollAmount) } : undefined
                })
            });
    ...
    const handleViewPayslip = () => {
        ...
        const monthNumber = (MONTH_NAMES.indexOf(attendanceMonth) + 1).toString().padStart(2, '0');
        const mString = `${attendanceYear}-${monthNumber}`;
        navigate(`/dashboard/payroll/payslips?month=${mString}&staffId=${viewingAttendanceStaff.id}`);
    };
        ...
        <div className="space-y-6 max-w-6xl mx-auto pb-10">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-slate-800">Payroll Run: {viewingAttendanceStaff.first_name} {viewingAttendanceStaff.last_name}</h1>

                        <p className="text-slate-500 text-sm">Employee ID: #{viewingAttendanceStaff.id} | Department: {viewingAttendanceStaff.department?.name || 'N/A'}</p>
                    </div>
                </div>
                                ...
                                <table className="w-full text-sm text-left">
                                    ...
                                    <tbody className="divide-y divide-slate-50">
                                        {existingPayrollItem ? (
                                            <>
                                                {/* Payments */}
                                                {existingPayrollItem.payments?.map((pay: any, idx: number) => (
                                                    <tr key={`pay-${idx}`} className="hover:bg-slate-50/50">
                                                        ...
                                                        <td className="px-6 py-4">
                                                            <div className="flex flex-col">
                                                                ...
                                                                <span className="text-xs text-slate-400">{pay.payment_method}</span>
                                                            </div>
                                                        </td>
                                                ...
                                                <tr className="hover:bg-slate-50/50 bg-slate-50/30">
                                                    <td className="px-6 py-4 font-medium text-slate-700">
                                                        {new Date(existingPayrollItem.created_at).toLocaleDateString()}
                                                    </td>
                                                    <td className="px-6 py-4">
                                                        <span className="font-semibold text-slate-700">Payslip Generated</span>
                                                    </td>
                                                    <td className="px-6 py-4 font-bold text-indigo-600">
                                                        RM {Number(existingPayrollItem.net_pay).toLocaleString()}
                                                    </td>
                                                    <td className="px-6 py-4">
